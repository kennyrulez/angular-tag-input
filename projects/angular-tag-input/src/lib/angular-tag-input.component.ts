import { Component, OnInit, HostBinding, Input, OnDestroy, Output, EventEmitter, ElementRef, HostListener, forwardRef, ViewChild } from '@angular/core';
...
@Component({
    ...
    providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => AngularTagInputComponent), multi: true }]
})
export class AngularTagInputComponent implements ControlValueAccessor, OnDestroy, OnInit {
    ...
    ngOnInit() {
        ...
        if (this.autocompleteObservable) {
            this.autocompleteSubscription = this.autocompleteObservable.subscribe(value => (this.autocompleteResults = value));

            this.tagInputSubscription = this.tagInputField.valueChanges.subscribe(() => (this.autocompleteResults = []));
        } else {
    ...
    showAutocomplete(): boolean {
        return (
            this.autocomplete &&
            ((this.autocompleteItems && this.autocompleteItems.length > 0) || (this.autocompleteResults && this.autocompleteResults.length > 0)) &&
    ...
    private _isTagUnique(tag: any): boolean {
        ...
        const valueToCheck = typeof tag === 'object' ? tag[this.displayBy] : tag;
        for (const current of this.tagsList) {
            if (typeof current === 'object' && current[this.displayBy] === valueToCheck) {
                return false;
            }
            if (current === valueToCheck) {
                return false;
            }
        }
    ...
    private _isTagAutocompleteItem(tag: any): boolean {
        ...
        for (const current of this.autocompleteItems) {
            if (typeof current === 'object' && current[this.autocompleteDisplayBy] === valueToCheck) {
                return false;
            }
            if (current === valueToCheck) {
                return false;
